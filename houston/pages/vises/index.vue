<template>
    <main class="flex flex-col gap-8">
        <hgroup class="space-y-1">
            ...
            >
                Тут вы можете управлять визами
            </h2>
        </hgroup>
        <div class="flex justify-between">
            ...
            <NuxtLink :to="{name: 'vises-id', params: {id: 'new'}}">
            ...
            >
                <template #buttons="{item}">
                    <TairoTableCell class="px-6 py-4 flex justify-end">
                        <NuxtLink
                            :to="{name: 'vises-id', params: {id: item?.id}}"
...
const service = useService('vises', 'vises');
...
const { data, isPending, refetch } = useQuery({
    queryKey: ['vises', JSON.stringify(params.value)],
    queryFn: async () => {
