...
const Header: React.FC<HeaderProps> = () => {
    ...
    return (
        ...
        className='h-[20vh] flex items-center justify-between px-[15px] lt:max-w-lg lt:mx-auto lt:px-0 md:max-w-[745px] lg:max-w-[980px] xl:max-w-[1180px] xxl:max-w-7xl'>
                ...
                <Link href={'/about'} locale={locale} className={`text-[#fff] text-[13px] font-semibold duration-300 md:text-[15px] lg:cursor-pointer lg:hover:text-[var(--main-color-two)] lg:text-[17px] xl:text-lg xxl:text-xl ${asPath == '/about' ? 'text-[var(--main-color-two)]' : 'text-[#fff]'}`}>{t('header.about')}</Link>
                </nav>
                <Language/>
        </motion.header>
    );
